...
/** Soft cues → category name patterns (custom or default). */
...
/**
 * Prefer the user's custom/default categories when the message clearly points
 * at one (e.g. "School fees" → "School"). Falls back to the AI suggestion.
 */
export function resolveCategory(
  ...
  const allowed =
    categories.length > 0
      ? categories
      : (DEFAULT_EXPENSE as unknown as string[]);
  ...
  return allowed.find((c) => c === "Other") || allowed[0] || "Other";
}
